...
import cgy.util.filesystem : BinaryFile;
import cgy.util.memory : BinaryReader, BinaryWriter;
    ...
    void serialize(BinaryWriter writer, int[HalfEdge] halfEdgeMap) {
        ...
        foreach(idx, he ; halfEdges) {
            if(he in halfEdgeMap) {
                ...
                LogError("Error at ", siteId, ":", idx, " of ", halfEdges.length, "/", halfEdgeMap.keys.length);
                BREAKPOINT;
            }
        }
    }
